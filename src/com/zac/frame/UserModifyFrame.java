...
public class UserModifyFrame extends JFrame {
	 ...
	 * Create the frame.
	 */
	public UserModifyFrame(UserBean user) {
		this.user = user;
	...
	//填充控件方法实现
	private void updateContent(UserBean user) {
		textFieldID.setText(String.valueOf(user.getUserID()));
		textFieldTitle.setText(user.getUserTitle());
		textFieldClass.setText(String.valueOf(user.getUserClassification()));
		textFieldSecurityPolicy.setText(String.valueOf(user.getUserSecurityPolicy()));
		textFieldPrio.setText(String.valueOf(user.getUserPriority()));
	}
	...
	protected void do_savebuttom_actionPerformed(ActionEvent arg0) {
        ...
        tempUser.setUserTitle(userTitle);
        tempUser.setUserClassification(Integer.parseInt(userClass));
        tempUser.setUserSecurityPolicy(Integer.parseInt(userSecurityPolicy));
        tempUser.setUserPriority(Integer.parseInt(subPriority));
        
        int result = UserOp.update(tempUser);
        if (result >= 0) {
        	TrafficManagerPanel.refreshUserTable();
            JOptionPane.showMessageDialog(null, "用户类型修改成功！");
            return;
        } else {
            JOptionPane.showMessageDialog(null, "用户类型修改失败！");
            return;
        }
	}
}
