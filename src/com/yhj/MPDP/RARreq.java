...
public class RARreq {
	...
	public String reqBW(CopsprPdpImpl pdp, Socket[] pepSockets, String params,
			Socket[] peerBB, int[] peerID, int bbID, String[] statusList,
		...
		if (requestedBW < 0)
			return "REQUESTED BW INPUT ERROR";
		...
		try {
			...
			ResultSet res_SLA = stmt2
					.executeQuery("select *,current_date() from SLA where sla_id="
							+ sla);
			...
			if (requestedBW > bwOld) {
				try {
					...
					log.println(now
							+ " - SLA : "
							+ sla
							+ " attempted to request more bandwidth that it is allocated.");
					log.close();
				} catch (IOException ex) {
					System.err.println("IOException: " + ex.getMessage());
				}
				return "Requested Rate of "
						+ requestedBW
						+ " is above the limit. A maximum bandwidth request of "
						+ bwOld + " is allowable. Request Rejected.";
			}

			// Check if the current domain can handle the bandwidth to be
			// requested

			ResultSet checkDomain = stmt2
					.executeQuery("select * from Capacity where Domain='"
							+ currentDomain + "'");
			...
			if (requestedBW > availCapacity) {
				try {
					...
					PrintWriter log = new PrintWriter(new FileWriter(
							"./logs/BBLog.txt", true));
					log.println(now + " - SLA : " + sla
							+ " attempted to request more bandwidth than the "
							+ currentDomain + " can handle.");
					log.close();
				} catch (IOException ex) {
					System.err.println("IOException: " + ex.getMessage());
				}
				return "Request Failed     . Requested Rate of "
						+ requestedBW
						+ " is above the Domain's capacity. Maximum available capacity is "
						+ availCapacity;
			}

			ResultSet res_codepoint = stmt1
					.executeQuery("select dscp from codepoint where service_type='"
							+ service_type + "'");
		...
		System.out.println("The current domain : " + currentDomain
				+ " can handle the requested rate of " + requestedBW);

		// Following lines of code determines if requested RAR falls within SLA
		// time limits
		...
		if (!msd.compareDate(slaStart, startdate, sla_stime, req_starttime)) {
			return "Invalid Request. RAR cannot start before SLA start date ("
					+ slaStart + ").";
		}
		...
		if (msd.compareDate(slaEnd, startdate, sla_etime, req_starttime)) {
			return "Invalid Request. RAR cannot start after SLA end date ("
					+ slaEnd + ").";
		}
		...
		if (msd.compareDate(slaEnd, enddate, sla_etime, req_endtime)) {
			return "Invalid Request. RAR cannot end after SLA end date ("
					+ slaEnd + ").";
		}
		...
		if (checkDestination.equals("DST Domain")) {
			...
			try {
				...
				stmt1.executeUpdate("update Capacity set availCapacity="
						+ newAvail + " where Domain='" + currentDomain + "'");

				System.out.println("Capacity for " + currentDomain + " is now "
						+ newAvail);

				System.out.println();

				int newBW = bwOld - requestedBW;
				newRAR = udb.insertRAR(sla, startdate, req_starttime, enddate,
						req_endtime, requestedBW, src, dst);
				...
				stmt1.executeUpdate("insert Flows values(" + newRAR + ",'"
						+ currentDomain + "'," + requestedBW + ",'"
						+ req_starttime + "','" + startdate + "','"
						+ req_endtime + "','" + enddate + "')");

			} catch (SQLException e) {
				System.out.println(e);
			}
			// Send new configuration data to relevent PEP in the current
			// domain.
			rc.sendcops(pdp, pepSockets, currentDomain, (short) 1, src, dst);

			// For the case where the source & destination of the request are in
			// the same domain

			if (theSRC.equals("SRC Domain")) {
				return "The Request is Successful. The new RAR has ID : "
						+ newRAR + " and is valid from " + req_starttime + " "
						+ startdate + " until " + req_endtime + " " + enddate;
			}

			else
				return "Request Successful:" + newRAR;
		}
		...
		/*
		 * The following code implies that an inter-domain request has been
		 * made. Therefore connections to peer BB's are required
		 */
		...
		if (checkDestination.substring(0, 7).equals("interBB")) {
			...
			// Connect to peer BB
			transitRet = sibbreq.connectPeer(currentDomain,
					checkDestination.substring(8), peerBB, statusList, sla,
					startdate, req_starttime, enddate, req_endtime,
					requestedBW, src, dst);

			// If the peer BB responds that the inter-domain request has been
			// successful

			if (transitRet.substring(0, 18).equalsIgnoreCase(
					"Request Successful")) {
				try {
					Connection conn;
					...
					System.out.println("Capacity for " + currentDomain
							+ " is now " + newAvail);

					stmt1.executeUpdate("insert Flows values("
							+ transitRet.substring(19) + ",'" + currentDomain
							+ "'," + requestedBW + ",'" + req_starttime + "','"
							+ startdate + "','" + req_endtime + "','" + enddate
							+ "')");
					System.out.println("Traffic flow belonging to RAR : "
							+ transitRet.substring(19) + " has been added to "
							+ currentDomain);
		...
		if (theSRC.equals("SRC Domain")
				&& transitRet.substring(0, 18).equals("Request Successful")) {

			rc.sendcops(pdp, pepSockets, currentDomain, (short) 1, src, dst);
			return "The request is successful. The RAR ID for the new inter-domain flow is : "
					+ transitRet.substring(19)
					+ ". It is valid from "
					+ req_starttime
					+ " "
					+ startdate
		...
		else if (theSRC.equals("SRC Domain")
				&& transitRet.equals("Request Failed")) {
			return "Request Failed";
		}

		else if (transitRet.length() > 14
				&& transitRet.substring(0, 14).equals("Request Failed")) {
			return transitRet;
		}
