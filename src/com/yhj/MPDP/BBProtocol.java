...
public class BBProtocol {
	...
	// // Request发给MPDP，
	// while (echo == 0) {
	// echo = mpep.DECEcho(); // 等待CPDP返回DEC
	// }
	// if (echo == 1) {
	// CopsDEC decs = mpep.getDEC();
	// System.out.println("MPEP received DEC From MPDP : " + decs);
	// /*
	// * PDP should send message to PEP about the resource allocation strategy
	// * PDP should also send message to BBClient whether it can get the
	// resource
	// */
	// CopsCAT cat = new CopsCAT((short) 1, (short) 3);
	// //send a CAT message instead to check the connection between PDP and PEP
	// pdp.sendMessage(cat); // 转发来自MPDP的DEC
	// }
	// } catch (Exception e) {
	// e.printStackTrace();
	// }
