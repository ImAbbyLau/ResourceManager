...
public class updateDB {
	...
	public void dropPEP(String currentDomain,Socket[] pepSockets,CopsprPdpImpl pdp) {
		...
		try {
			...
			ResultSet res = st1.executeQuery("select sindex from PEP where Domain='"+currentDomain+"' AND status='ON'");
			while(res.next()) {
	...
	public short getPDPport(String pepID) {
		...
		try {
			...
			Connection conn;
			conn = DriverManager.getConnection("jdbc:mysql://" + mysqlURL + "/CPR","root","111111");
			Statement stmt1 = conn.createStatement();

			//stmt1.executeUpdate("insert into PEP values('" + pepID+ "','" + pepAddy + "'," + pepPort + ")");
			ResultSet res = stmt1.executeQuery("select Domain from CPEP where CPEP='" + pepID + "'");
			String Domain="";
			while (res.next()) {
				Domain = res.getString("Domain");
			}
			res = stmt1.executeQuery("select pdpPort from CPDP where Domain='" + Domain + "'");
			while (res.next()) {
