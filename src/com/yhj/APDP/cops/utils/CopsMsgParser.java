...
public class CopsMsgParser {
  ...
  public CopsMessage nextMessage() throws CopsException, IOException {
    ...
    System.out.println("vf : " + vf);    //Add By Myself
    System.out.println("version : " + version);    //Add By Myself
    byte flag = (byte)(vf - (version << 4));
    byte opcode = dis.readByte();
    short clientType = dis.readShort();
    int msglen = dis.readInt();
    System.out.println("msglen : " + msglen);    //Add By Myself
    byte[] msg = new byte[msglen-8];
    dis.read(msg, 0, msglen-8);
    System.out.println("msg : " + msg);    //Add By Myself
    System.out.println("opcode : " + opcode);    //Add By Myself
    cch = new CopsCommonHeader(version, flag, opcode, clientType, msglen);

    switch (opcode) {
    case CopsMessage.REQ:{
     result = parseREQ(cch, msg);
     System.out.println("afterreq : " + result);    //Add By Myself
     break;
    }
    case CopsMessage.DEC:{
    	System.out.println("opmsg : " + msg);    //Add By Myself
        result = parseDEC(cch, msg);
        System.out.println("aftermsg : " + result);    //Add By Myself
        break;
    }
