...
public class RecvThread extends Thread {
  ...
  RecvThread(Socket s, CopsMsgHandler cmh, CopsMsgParser cmp,Socket[] pepSockets) {
    this.cmp = cmp;
    this.cmh = cmh;
    this.s = s;
  ...
  RecvThread(Socket s, CopsMsgHandler cmh, CopsMsgParser cmp) {
  	this.cmp=cmp;
  	this.cmh=cmh;
  	this.s=s;
  }
  ...
  public void close() {
	...
	public void run() {
		thread = this;
		System.out.println("CopsMPEP.RecvThread.run started");
		Thread thisThread = Thread.currentThread();
		while (thread == thisThread) {
			try {
				CopsMessage msg = cmp.nextMessage();
				if (msg == null)
					throw new CopsException("null message");
